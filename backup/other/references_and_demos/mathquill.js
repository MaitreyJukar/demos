    ...
    var manageTextarea = (function () {
        ...
        return function manageTextarea(el, opts) {
            ...
            function onKeydown(e) {
                ...
                var $currentFrownParent = $(this).find('.cursor').parent().parent().hasClass('custom-frown-parent') ? $(this).find('.cursor').parent().parent() : null,
                    paddingForSymbol = 5;

                if ($currentFrownParent !== null) {
                    var width = $currentFrownParent.find('.non-leaf').width();
                    if (width >= 5) {
                        width += paddingForSymbol;
                    }
                    $currentFrownParent.find('.custom-frown').css({ width: width });
                }
                ...
                if ($currentlraParent !== null) {
                    var width = $currentlraParent.find('.non-leaf').width() + paddingForSymbol;
                ...
                var $currentlraParent = $(this).find('.cursor').parent().parent().hasClass('custom-parent') ? $(this).find('.cursor').parent().parent() : null;
                if ($currentlraParent !== null) {
                    var width = $currentlraParent.find('.non-leaf').width() + paddingForSymbol;
                    if (width >= 14) {
    ...
    ********************************************/
    ...
    textarea = textareaSpan.children();
        ...
        */
        ...
        //Binds double-click event to select entire text
        jQ.unbind('dblclick', selectAll).bind('dblclick', selectAll);

        //Binds touchstart, touchend and touchmove event to select entire text on tap and hold
        jQ.unbind('touchstart.mathquill-selection').bind('touchstart.mathquill-selection', function (e) {
            
            jQ.find('textarea').focus();
            ...
            jQ.unbind('touchmove.mathquill-selection').bind('touchmove.mathquill-selection', function (e) {
                if (touchMoveCount > 10) {
                    touchMove = true;
                    jQ.unbind('touchmove.mathquill-selection');
                }
                else {
                    touchMoveCount++;
                }
            ...
            jQ.unbind('touchend.mathquill-selection').bind('touchend.mathquill-selection', function (e) {
                touchEnd = true;
                jQ.unbind('touchmove.mathquill-selection');
                jQ.unbind('touchend.mathquill-selection');
            });

        });
        //Selects the entire text on double-click or tap-and-hold
        function selectAll(e) {
            cursor.parent.bubble('onKey', 'Ctrl-A', e);
        }
      ...
      .prepend('<span class="selectable">$' + root.latex() + '$</span>');
        ...
        var textareaManager = manageTextarea(textarea, {
            ...
            paste: function (text) {
                ...
                if (text && text.indexOf('\\sum') >= 0) {
                    text = text.replace(' ^{', ' {');
                    text = text.replace('}_{', '}{');
                }
                if (text && text.indexOf('\\prod') >= 0) {
                    text = text.replace(' ^{', ' {');
                    text = text.replace('}_{', '}{');
                }
    ...
    var RootMathBlock = P(MathBlock, function (_, _super) {
        _.latex = function () {
            ...
            // returnLatex = returnLatex.replace(/(\\le\s*)|(\\ge\s*)/,function($1, $2, $3) {
            // var returnString = $1;
...
LatexCmds.nthroot = P(SquareRoot, function (_, _super) {
    ...
    LatexCmds.left = P(MathCommand, function (_) {
        _.parser = function () {
            ...
            return optWhitespace.then(regex(/^(?:[([|]|\\\{|\\lceil|\\lfloor)/))
      .then(function (open) {
...
LatexCmds.vector = P(MathCommand, function (_, _super) {
    ...
    *********************************/
    ...
    var FrownSymbol = P(MathCommand, function (_, _super) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="custom-frown-parent "><span class="custom-frown-holder"><object type="image/svg+xml" class="custom-frown" width:"100%" height:"100%"></object>' + '</span>';
        ...
        */
        ...
        function moveUpDown(self, dir) {
            ...
            if (self[R][dir]) self.insAtLeftEnd(self[R][dir]);
                ...
                do {
                    ...
                    else if (cursorMainBlock.length > 0 && cursorFromBlock.length > 0 && dir === 'up' && (cursorParent.children().length - 1 === cursorParent.children('.cursor').index()) && self.jQ.parent().children('.mathquill-main-block').length === 0) { //movement from 'from' to 'right parent'
                        self.moveRight(cursorMainBlock);
                        self.moveRight(cursorMainBlock);
                        break;
                    }
                    else if (self[L] && self[L].jQ.hasClass('mathquill-main-block')) {   //movement from 'right parent' to 'to'
                        if (dir === 'up') {
                    ...
                    else if (self.parent.jQ.hasClass('mathquill-editable') && (self.parent.jQ.find('.mathquill-main-block').index() === self.parent.jQ.find('.cursor').index() + 1)) {
                        self.moveRight(self.parent.jQ.find('.to'));
                        self.moveRight(self.parent.jQ.find('.to'));
                    }
        ...
        _.writeLatex = function (latex) {
            ...
            if (block) {
                ...
                var customFrownParent = mathquillParent.find('.custom-frown-parent'),
                    customFrownNonLeafWidth = customFrownParent.find('.non-leaf').width(),
                    customFrownObject = customFrownParent.find('object');
                ...
                if (customFrownParent.length > 0) {
                    if (customFrownNonLeafWidth >= 5) {
                        customFrownObject.width(customFrownNonLeafWidth);
                    }
                }
    ...
    jQuery.fn.mathquill = function (cmd, latex) {
          ...
          block = blockId && MathElement[blockId];
                        if (block) {
                            if (latex && latex.indexOf('\\sum') >= 0) {
                                latex = latex.replace('m^{', 'm{');
                                latex = latex.replace('}_{', '}{');
                            }
                            if (latex && latex.indexOf('\\prod') >= 0) {
                                latex = latex.replace('d^{', 'd{');
                                latex = latex.replace('}_{', '}{');
                            }
      ...
      block = blockId && MathElement[blockId];
                ...
                if (latexData && latexData.indexOf('\\sum') >= 0) {
                    latexData = latexData.replace('m {', 'm^{');
                    latexData = latexData.replace('}{', '}_{');
                    if (latexData && latexData.indexOf('\\prod') >= 0) {
                        latexData = latexData.replace('d {', 'd^{');
                        latexData = latexData.replace('}{', '}_{');
                        return latexData;
                    }
                    return latexData;
