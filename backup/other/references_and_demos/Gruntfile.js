module.exports = function (grunt) {
    ...
    */
    ...
    /*
    * Generates the union of two arrays using the property passed
    * @method _unionArrays
    ...
    */
    var _unionArrays = function (arr1, arr2, prop1) {
    ...
    */
    ...
    var _vendorCheck = function (filePath) {
        return (filePath.indexOf('vendor') !== -1) || (filePath.indexOf('plugin') !== -1);
    }
   ...
   */
    grunt.registerTask('build-interactive-engine', function (mapParam) {
        ...
        var basepath = pathData._basePath,
            ...
            jsMinifiedFilePath = pathData._basePath + pathData._commonPath + pathData._outputDir + pathData._scriptPath + jsMinifiedFileName,
            jsFiles = {},
            jsFilesToMinify = [],
   ...
   */
    grunt.registerTask('build-common', function (gruntData) {
        ...
        var batchIterationCounter = grunt.config("batch-iteration.counter");
   ...
   */
    grunt.registerTask("minify_json", function () {
        ...
        var minFiles = grunt.file.expand({ filter: 'isFile' }, [jsonPath, distJsonPath, qaImagesJson]);
    ...
    */
    ...
    /*
    * Reset step no for css and js files.
    * @method resetStepCounterForCssJs
    */
    var resetStepCounterForCssJs = function () {
        var _initialStep = _STEPS[0];
        //grunt.config("cssFiles.counter", _initialStep);
        //grunt.config("jsFiles.counter", _initialStep);
    }
    ...
    */
    grunt.registerTask("build_css", function () {
        var cssFiles = {}, cssMinFiles = {}, fileNames = {},
            counter = 0, stepsCounter = 0, themeCounter = 0,
            stepsLength = _STEPS.length, themeLength = _THEMES.length, destFileName, filesToGrunt = {},
            fileType, curStep, cssFilesInConfig, cssFilesLength, currentFileStep, curFile, curFilePath, curMinFilePath, concatPath, concatFileName;
    ...
    */
    grunt.registerTask("build_common_css", function () {
        var cssFiles, cssMinFiles = {}, fileNames,
            counter = 0, stepsCounter = 0, themeCounter = 0,
            stepsLength = _STEPS.length, themeLength = _THEMES.length, destFileName, filesToGrunt = {},
            fileType, cssFilesInConfig = [], cssFilesLength, currentFileStep, curFile, curFilePath, curMinFilePath, concatPath = [], concatFileName,
            cssFilesForComponentsTheme1 = [], cssFilesForComponentsTheme2 = [], cssFilesForCommonTheme1 = [], cssFilesForCommonTheme2 = [], cssFilesForInteractive = {},
            concatInteractivePath, componentFiles;
   ...
   */
    grunt.registerTask("build_js", function () {
        ...
        for (; counter < jsFilesLength; counter++) {
            ...
            if (curFile.basePath === 'COMMON_SCRIPT_FONTS') {
                continue;
            }
    ...
    */
    grunt.registerTask("updateConfig_js", function () {
        var jsConfigJson, templateConfigJson, curStep, jsJsonToDel, configCounter, stepsLength = _STEPS.length, stepsCounter = 0, fileBasePath;
   ...
   */
    grunt.registerTask("build_common_js", function () {
        var jsFiles = [], jsMinFiles = [], jsFileNames = [],
            counter = 0, stepsCounter = 0, themeCounter = 0,
            stepsLength = _STEPS.length, themeLength = _THEMES.length,
            jsFilesInConfig = [], jsFilesInConfigTheme1 = [], jsFilesInConfigTheme2 = [], jsFilesLength, curFile, currentFileStep,
            templateFilesInConfig, templateFilesInConfigTheme1, templateFilesInConfigTheme2, templateFilesLength, filesToMinify = [[]],
            jsFileType, templateFileType, componentFiles, jsFilesForComponents = {}, jsFilesForInteractive = {}, jsFilesForCommonTheme1 = [], jsFilesForCommonTheme2Step1 = [], jsFilesForCommonTheme2Step2 = [];
        var files = {},
            concatPath = pathData._basePath + pathData._appName + pathData._interactivitiesPath + pathData._outputDir + "js" + pathData._interactivitiesPath,
            concatFileName, destFileName, concatTemplate;
        ...
        jsFilesInConfig = _unionArrays(jsFilesInConfigTheme1, jsFilesInConfigTheme2, 'url');
        templateFilesInConfig = _unionArrays(templateFilesInConfigTheme1, templateFilesInConfigTheme2, 'url');
    ...
    */
    grunt.registerTask("updatePlayerConfig_js", function () {
        ...
        for (var themeCounter = 0; themeCounter < 2; themeCounter++) {
            jsJsonToDel = grunt.config("jsCommonFiles.fileNames.group1");
            var curMinJSFile = {};
            ...
            for (var i = 0; i < jsJsonToDel.length; i++) {
                for (var j = 0; j < jsConfigJson[themeCounter].length; j++) {
                    ...
                    if (typeof jsConfigJson[themeCounter][j] !== 'undefined') {
                ...
                for (var j = 0; j < templatesConfigJson[themeCounter].length; j++) {
                    if (typeof templatesConfigJson[themeCounter][j] !== 'undefined') {
                        if (templatesConfigJson[themeCounter][j].url === jsJsonToDel[i].url) {
                            templatesConfigJson[themeCounter].splice(j, 1);
                            j--;
                        }
                    }
                }
    ...
    */
    grunt.registerTask("write_configJson_to_file", function () {
        //var _tempConfigFilePath = configFilePath.substr(0, configFilePath.length - 5) + "-grunt.json";// subtract 5 for ".json"
        var tempPathHolder = configFilePath.split(pathData._appName + pathData._interactivitiesPath);
        var _newConfigFilePath = tempPathHolder[0] + pathData._appName + pathData._interactivitiesPath + pathData._outputDir + tempPathHolder[1];
