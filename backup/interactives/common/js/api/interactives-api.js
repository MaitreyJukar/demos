var Interactivities = function (playerOptions) {
    ...
    */
    ...
    /**
    * Stores the player config data
    *
    * @property _playerConfig
    * @default null
    * @private
    */
    ...
    */
    this.initialize = function (playerOptions) {
        ...
        if (playerOptions && playerOptions.staticPath) {
            this._coreStaticPath = playerOptions.staticPath;
        }
        if (playerOptions && typeof playerOptions.minify !== 'undefined' && playerOptions.minify !== null) {
    ...
    */
    this._loadDependencies = function () {
        ...
        if (this._minify === false) {
            ...
            {
                'type': 'json',
                'path': basepath + 'common/data/player-config.json',
                'id': 'player-config'
            },
            {
                'type': 'css',
        ...
        successCallback = function (event) {
            ...
            //Setting player config data to api property, and further passing this in load interactive
            if (self._minify) {
                self._playerConfig = $.extend({}, MathInteractives.Common.Player._configData);
                delete MathInteractives.Common.Player._configData;
            }
            else {
                self._playerConfig = event.preloader.getPreloadData()['player-config'].data;
            }
    ...
    this.loadInteractive = function (interactiveOptions) {
        ...
        if (!isInteractivitylLoaded) {
            ...
            if (typeof self.getInteractiveOptions()[interactiveOptions.interactiveGuid] !== 'undefined') {
                ...
                interactiveOptions.coreStaticPath = interactiveOptions.staticPath || self._coreStaticPath;
                ...
                interactiveOptions.playerConfig = self._playerConfig;
                interactiveOptions.initialState = interactiveOptions.initialState;
    ...
    */
    this.interactiveSuccess = function (callbackData, interactiveObj, interactiveGuid) {
        ...
        if (callbackData && callbackData.loadedStepNumber && callbackData.loadedStepNumber === 1) {
            ...
            interactiveObj.model.off('pop-out').on('pop-out', $.proxy(this.popoutButtonTriggered, this));
    ...
    this.saveDataTriggered = function (event) {
        ...
        var saveState = this.getCurrentStateData(event.interactiveGuid),
            ...
            saveStateObject = {
                ...
                saveState: saveState,
                success: $.proxy(function (response) { this._saveStateSuccessCallback(response) }, self),
                error: $.proxy(function (response) { this._saveStateErrorCallback(response) }, self),
                complete: $.proxy(function (response) { this._saveStateCompleteCallback(response) }, self)
            };
        ...
        if (clientApi !== null && typeof clientApi !== 'undefined' && typeof clientApi.saveInteractiveState !== 'undefined') {
            ...
            console.log("API is not implemented");
        }
    }

    this.popoutButtonTriggered = function (event) {
        var clientApi = this._options.clientApi;
        if (clientApi !== null && typeof clientApi !== 'undefined' && typeof clientApi.saveInteractiveState !== 'undefined') {
            clientApi.popoutInteractive(event.interactiveGuid, event.containerId, event.data);
        }
        else {
            console.log("API is not implemented");
        }
    ...
    * @private
    */
    this._saveStateSuccessCallback = function (response) {
        //ToDo
        //will be listened in player.
        var interactiveGuid, interactiveObject;
        interactiveGuid = response.interactiveGuid;
        interactiveObject = this._interactivityLoadController.getInteractivity(interactiveGuid);
        interactiveObject.model.trigger('save-state-success', response.msg);
    }
    ...
    * @private
    */
    this._saveStateErrorCallback = function (response) {
        var interactiveGuid, interactiveObject;
        interactiveGuid = response.interactiveGuid;
        interactiveObject = this._interactivityLoadController.getInteractivity(interactiveGuid);
        interactiveObject.model.trigger('save-state-error', response.msg);
    }
    ...
    */
    this.saveImageTriggered = function (event) {
        var clientApi = this._options.clientApi;
       // clientApi = (clientApi === null) ? DEInteractivesClient : clientApi;
        //Calling DEApi funtion (if defined) to return the json object
        if (clientApi !== null && typeof clientApi !== 'undefined' && typeof clientApi.saveImage !== 'undefined') {
            if (clientApi.bZeus) {
                clientApi.saveImage(event.fileName, event.base64Image);
            }
            else {
                clientApi.saveImage({ "data": { "content": event.base64Image, "name": event.fileName } }).always(function (response) {
                    $.de.loadingComplete($("body"));
                    $.de.browse({
                        "actionType": 'add',
                        "assetTitle": event.fileName,
                        "assetGuid": response.assetGuid
                    });
                });
            }
        } else {
            alert("API function call not implemented");
        }
    }
    /**
    ...
    this.unloadInteractive = function (interactivityData) {
        ...
        if (typeof interactivityInfo !== 'undefined' && interactivityInfo.isInteractivityLoaded) {
            ...
            if (interactivityInfo && interactivityInfo.interactivityObj) {
                // Trigger custom event on unloading the interactive
                interactivityInfo.interactivityObj.trigger(MathInteractives.Common.Player.Views.Player.Events.UNLOAD_INTERACTIVE);
                ...
                delete this._interactiveOptions[interactivityData.interactiveGuid];
    ...
    this.getCurrentStateData = function (interactiveGuid) {
        ...
        if (typeof interactiveInfo !== 'undefined') {
            ...
            if (typeof interactiveInfo.interactivityObj.interactiveModel.getCurrentStateData === 'undefined') {
                ...
                player = interactiveInfo.interactivityObj;
                playerModel = player.model;
                initialState = player.interactiveModel.getCurrentStateData();
                currentState = $.parseJSON(initialState);
                //Set the previous save state after ignoring the values in the attributesToIgnore array
                previousSaveState = playerModel.ignoreAttrsForSaveState(currentState, player.interactiveModel.get('attributesToIgnore'));
                // perform backup operation
                playerModel.performBackupSaveState();
                playerModel.set('previousSaveState', previousSaveState);
                return currentState;
            }
        }
        else {
