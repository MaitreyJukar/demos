...
(function (window, document, undefined) {
    ...
    _html2canvas.Util.getCSS = function (el, attribute, index) {
        var val, isBackgroundSizePosition = attribute.match(/^background(Size|Position)$/), BrowserCheck = MathInteractives.Common.Utilities.Models.BrowserCheck;
        ...
        if (BrowserCheck.isIE && parseInt(BrowserCheck.browserVersion) < 10 && attribute === "backgroundPosition") {
            val = el.currentStyle.backgroundPosition;
        }
        else if (BrowserCheck.isIE && attribute.indexOf("border") > -1 && attribute.indexOf("Width") > -1 && (el.tagName === "TD" || el.tagName === "TH")) {
            val = Math.ceil(parseFloat(val)) + "px";
        }
    ...
    _html2canvas.Util.BackgroundPosition = function (el, bounds, image, imageIndex, backgroundSize) {
        ...
        return {
            left: result[0] - (bounds.left - $(el).offset().left),
    ...
    */
    ...
    (function () {
        ...
        var reGradients = [/^(-webkit-linear-gradient)\(([a-z\s]+)([\w\d\.\s,%\(\)]+)\)$/, /^(-o-linear-gradient)\(([a-z\s]+)([\w\d\.\s,%\(\)]+)\)$/, /^(-webkit-gradient)\((linear|radial),\s((?:\d{1,3}%?)\s(?:\d{1,3}%?),\s(?:\d{1,3}%?)\s(?:\d{1,3}%?))([\w\d\.\s,%\(\)\-]+)\)$/, /^(-moz-linear-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?))([\w\d\.\s,%\(\)]+)\)$/, /^(-webkit-radial-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?)),\s(\w+)\s([a-z\-]+)([\w\d\.\s,%\(\)]+)\)$/, /^(-moz-radial-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?)),\s(\w+)\s?([a-z\-]*)([\w\d\.\s,%\(\)]+)\)$/, /^(-o-radial-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?)),\s(\w+)\s([a-z\-]+)([\w\d\.\s,%\(\)]+)\)$/, /^(linear-gradient)\(([a-z\s]+,)*([\w\d\.\s,%\(\)]+)\)$/];
        ...
        * Note: Extra check for W3C gradient has been added
        * @author(original) Nikalv
        * @author(changes i.e W3C compliance linear-gradient) Charles
        */
            ...
            if (m1) {
                switch (m1[1]) {
                    ...
                    case '-webkit-gradient':
                        ...
                        gradient = {
                            type: m1[2] === 'radial' ? 'circle' : m1[2], // TODO: Add radial gradient support for older
                            // mozilla definitions
                            x0: 0,
                            y0: 0,
                            x1: 0,
        ...
        */
        _html2canvas.Generate.listGreekAlpha = function listGreekAlpha(number) {
            var result,
                greekAlphabets = ['Α', 'Β', 'Γ', 'Δ', 'Ε', 'Ζ', 'Η', 'Θ', 'Ι', 'Κ', 'Λ', 'Μ', 'Ν', 'Ξ', 'Ο', 'Π', 'Ρ', 'Σ', 'Τ', 'Υ', 'Φ', 'Χ', 'Ψ', 'Ω'],
    ...
    _html2canvas.Parse = function (images, options) {
        ...
        var element = ((options.elements === undefined) ? document.body : options.elements[0]), // select body by
        // default
        ...
        function renderTextDecoration(ctx, text_decoration, bounds, metrics, color) {
            switch (text_decoration) {
                case "underline":
                    ...
                    // TODO As some browsers display the line as more than 1px if the font-size is big, need to take that
                    // into account both in position and size
        ...
        function setZ(zIndex, parentZ) {
            // TODO fix static elements overlapping relative/absolute elements under same stack, if they are defined
            // after them
            var newContext;
            if (!parentZ) {
        ...
        function renderBackgroundImage(element, bounds, ctx) {
            ...
            /*var strTransform = $(element).css("transform");
            if(strTransform && strTransform !== "none")
            {var arrTransform = strTransform.substring(strTransform.indexOf("(") + 1, strTransform.length - 1).split(",
            ");
            ctx.transform.apply(window, arrTransform);
            }*/
        }
    ...
    _html2canvas.Preload = function (options) {
        ...
        var images = {
            ...
            numTotal: 0,
            cleanupDone: false
        }, pageOrigin, methods, i, count = 0, element = options.elements[0] || document.body, doc = element.ownerDocument, domImages = [], imgLen = 0,
        // // TODO probably should limit it to images present in the element only
        ...
        if (options.elements) {
            for (i = 0; i < options.elements.length; i++) {
                if (options.elements[i].nodeName.toLowerCase() === "img") {
                    domImages.push(options.elements[i]);
                }
            }
            $(options.element).find('img').each(function () {
                domImages.push(this);
            });
        }
        ...
        function proxyGetImage(url, img, imageObj) {
            ...
            window[callback_name] = function (a) {
                ...
                // to work with IE<9  // NOTE: that the undefined callback property-name still exists on the window
                // object (for IE<9)
                try {
        ...
        function setImageLoadHandlers(img, imageObj, isCrossOrigin, src) {
            ...
            if (isCrossOrigin && src) {
                MathInteractives.Common.Utilities.getBase64FromImageURL(src, function onSuccess(imageBase64) {
                    ...
                    for (src in images) {
                        if (images.hasOwnProperty(src)) {
                            ...
                            if (typeof img === "object" && img.callbackname && img.succeeded === undefined) {
                                ...
                                // to work with IE<9  // NOTE: that the undefined callback property-name still exists on
                                // the window object (for IE<9)
                                try {
