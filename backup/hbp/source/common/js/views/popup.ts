...
export class Popup extends Backbone.View<PopupModelPckg.Popup> {
    ...
    constructor(attr: any = {}) {
        ...
        if (attr.el === void 0) {
            console.warn("WARNING! el attribute must be provided for 'Popup' view to work");
        }
        super(attr);
        ...
        switch (event.which) {
            case 9: // Tab key
                if (event.shiftKey && $focusables.index($(event.currentTarget)) === 0) {
                    console.info("[Popup] Shift tab on 1st elem, Focusing on last element");
                    ...
                    console.info("[Popup] Tab on last elem, Focusing on first element");
                    event.preventDefault();
                    event.stopPropagation();
