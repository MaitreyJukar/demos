...
(function() {
    'use strict';
    MathUtilities.Components.ColorPicker.Models.Colors = Backbone.Model.extend({

        "_valueRanges": {  // Specifying Minimum & Maximum possible values
            "rgb": {
                "r": [0, 255], // red
                "g": [0, 255], // green
                "b": [0, 255] // blue
            ...
            "Lab": {
                "L": [0, 100], // lightness
                "a": [-128, 127], // color opponent dimension
                "b": [-128, 127] // color opponent dimension
            },
            "XYZ": {
                "X": [0, 100], // color space
                "Y": [0, 100], // color space
        ...
        "XYZMatrix": { // Matrix is used for conversions.
        ...
        "luminance": { // values are used for conversion to RGB
            "r": 0.2126,
            "g": 0.7152,
            "b": 0.0722
        },
        ...
        "initialize": function() {
            ...
            this.options = {
                "color": 'rgba(204, 82, 37, 0.8)', // initial Values.
        ...
        "_setColor": function(colors, color, type, save, alpha) {
        ...
        "_saveAsBackground": function(RGB, rgb, alpha) {
        ...
        "_convertColors": function(type, colorObj) {
            ...
            if (type !== 'alpha') {
                ...
                colors.saveColor = RGB.r === SAVE.r && RGB.g === SAVE.g && RGB.b === SAVE.b ?
                    'web save' : RGB.r === SMART.r && RGB.g === SMART.g && RGB.b === SMART.b ? 'web smart' : '';
        ...
        "getClosestWebColor": function(RGB, val) {
            var out = {},
                tmp = 0,
                half = val / 2,
                n;
        ...
        "getHueDelta": function(rgb1, rgb2, nominal) {
            return (Math.max(rgb1.r - rgb2.r, rgb2.r - rgb1.r) + Math.max(rgb1.g - rgb2.g, rgb2.g - rgb1.g) +
                Math.max(rgb1.b - rgb2.b, rgb2.b - rgb1.b)) * (nominal ? 255 : 1) / 765;
        },

        "getLuminance": function(rgb, normalized) {
            var div = normalized ? 1 : 255,
                RGB = [rgb.r / div, rgb.g / div, rgb.b / div],
                luminance = this._instance.options.luminance,
                i;

            for (i = RGB.length; i--;) { // Relative Luminance, please refer to http://www.w3.org/WAI/GL/wiki/Relative_luminance
                RGB[i] = RGB[i] <= 0.03928 ? RGB[i] / 12.92 : Math.pow(((RGB[i] + 0.055) / 1.055), 2.4);
            }
            return ((luminance.r * RGB[0]) + (luminance.g * RGB[1]) + (luminance.b * RGB[2]));
        },

        "mixColors": function(topColor, bottomColor, topAlpha, bottomAlpha) {
            var newColor = {},
        ...
        "getWCAG2Ratio": function(lum1, lum2) {
            var ratio = 1;

            ratio = lum1 >= lum2 ? (lum1 + 0.05) / (lum2 + 0.05) : (lum2 + 0.05) / (lum1 + 0.05); // as per contrast ratio formula
        ...
        "rgb2hex": function(RGB) {
            return ((RGB.r < 16 ? '0' : '') + RGB.r.toString(16) + (RGB.g < 16 ? '0' : '') + RGB.g.toString(16) +
        ...
        "hex2rgb": function(HEX) {
            ...
            // IE7
            return {
                "r": parseInt(HEX[0] + HEX[HEX[3] ? 1 : 0], 16) / 255,  // Calculating value for Red
                "g": parseInt(HEX[HEX[3] ? 2 : 1] + (HEX[3] || HEX[1]), 16) / 255, // Calculating value for Green
                "b": parseInt((HEX[4] || HEX[2]) + (HEX[5] || HEX[2]), 16) / 255 // Calculating value for Blue
            };
        },
        ...
        "rgb2hsv": function(rgb) {
            ...
            s = r ? (chroma / r) : 0;
            return {
                "h": s < 1e-15 ? ((this._colors && this._colors.hsl && this._colors.hsl.h) || 0) : chroma ?
                    Math.abs(k + (g - b) / (6 * chroma)) : 0,
                "s": r ? (chroma / r) : ((this._colors && this._colors.hsv && this._colors.hsv.s) || 0),
                "v": r
            };
        },

        "hsv2rgb": function(hsv) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            var h = hsv.h * 6,
                s = hsv.s,
                v = hsv.v,
                i = ~~h,
        ...
        "hsv2hsl": function(hsv) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            var l = (2 - hsv.s) * hsv.v,
                s = hsv.s * hsv.v;
        ...
        "hsl2rgb": function(hsl) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            var h = hsl.h * 6,
                s = hsl.s,
                l = hsl.l,
        ...
        "rgb2cmy": function(rgb) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            return {
                "c": 1 - rgb.r,
                "m": 1 - rgb.g,
                "y": 1 - rgb.b
        ...
        "cmy2cmyk": function(cmy) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
        ...
        "cmyk2cmy": function(cmyk) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            var k = cmyk.k;
        ...
        "cmy2rgb": function(cmy) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            return {
                "r": 1 - cmy.c,
                "g": 1 - cmy.m,
                "b": 1 - cmy.y
        ...
        "rgb2cmyk": function(rgb, dependent) { // rgb2cmyk conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            var cmy = this.rgb2cmy(rgb);
            return this.cmy2cmyk(dependent ? cmy : (_colors.cmy = cmy));
        },

        "cmyk2rgb": function(cmyk, dependent) { // cmyk2rgb conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
        ...
        "XYZ2rgb": function(XYZ, skip) { // XYZ2rgb conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
        ...
        "rgb2XYZ": function(rgb) { // rgb2XYZ conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            var M = this._instance.options.XYZMatrix,
                r = rgb.r,
                g = rgb.g,
                b = rgb.b,
        ...
        "XYZ2Lab": function(XYZ) { // XYZ2Lab conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
        ...
        "lab2XYZ": function(Lab) { // lab2XYZ conversion. Please refer to http://www.easyrgb.com/index.php?X=MATH
            var R = this._instance.options.XYZReference,
                Y = (Lab.L + 16) / 116,
                X = Lab.a / 500 + Y,
                Z = Y - Lab.b / 200,
