...
(function () {
  ...
  min = Math.min,
  max = Math.max,
  // isIOS variable added by zeus
  userAgent = window.navigator.userAgent.toLowerCase(),
  isIOS = userAgent.indexOf("mobile") > -1 && !userAgent.indexOf("chrome") && userAgent.indexOf("safari") > -1;
    ...
    */
    ...
    // START - changes done by Zeus
    /*Custom height addition for floor and ceil symbol*/
    var updateCeilFloorSymbolHeight = function (mathquillParent, symbolToUpdate) {
        var currentParent = mathquillParent.find('.custom-'  + symbolToUpdate + '-parent'),
        customNonLeafHeight, customObject, MIN_HEIGHT = 17;
        if (currentParent.length > 0) {
            $(currentParent.find('> .non-leaf').get().reverse()).each(function(index){
                $(this).css({
                    "vertical-align": "middle"
                });
                customNonLeafHeight = $(this).height();
                if (customNonLeafHeight >= MIN_HEIGHT) {
                    customObject =  $(this).siblings('.custom-'  + symbolToUpdate + '-holder').find('svg.custom-'  + symbolToUpdate);
                    customObject.each(function(index) {
                        $(this).height(customNonLeafHeight);
                    });
                }
            });
        }
    }
    /*Custom floor and ceil symbol code end*/
    /*Custom width addition for frown*/
    var updateFrownSymbolWidth = function (mathquillParent) {
        var currentFrownParent = mathquillParent.find('.custom-frown-parent'),
            customFrownNonLeafWidth, MIN_WIDTH = 8,
            customFrownObject = $(currentFrownParent.find('svg.custom-frown').get().reverse());
        if (currentFrownParent.length > 0) {
            $(currentFrownParent.find('> .non-leaf').get().reverse()).each(function(index){
                $(this).css({
                    "min-width": "8px"
                })
                customFrownNonLeafWidth = Math.ceil(Number(window.getComputedStyle(this, null).getPropertyValue('width').replace('px', ''))) ||
                    Math.ceil(this.getBoundingClientRect().width);
                if (customFrownNonLeafWidth >= MIN_WIDTH) {
                    $(customFrownObject[index]).width(customFrownNonLeafWidth);
                }
            });
        }
    }
    /*Custom frown code end*/
    // END - changes done by Zeus
    ...
    var manageTextarea = (function () {
        ...
        return function manageTextarea(el, opts) {
            ...
            function popText(callback) {
                ...
                textarea.val('');
                // \u00B1  represents plus minus symbol.
                if (text && text !== '\u00B1') callback(text); /*changes done by Zeus*/
            }
            ...
            function onKeydown(e) {
                if (e.keyCode === 40 && e.openBracket === true) {
                    keydown = e;
                    keypress = null;
                    return;
                }
                // START - changes done by Zeus
                /*this code was preventing user from
                entering pipe symbol for absolute sign*/
                // if (e.keyCode === 220) {
                //     keydown = e;
                //     keypress = null;
                //     e.preventDefault();
                ...
                // START - changes done by Zeus
                // keycode 178 is to prevent raised to terms.
                if(e.keyCode === 178)
                {
                    $(this).mathquill('clearSelection');
                }

                /*Custom width addition for Long Right Arrow*/
                var $currentlraParent = $(this).find('.cursor').parent().parent().hasClass('custom-lra-parent') ? $(this).find('.cursor').parent().parent() : null,
                    PADDING_FOR_SYMBOL = 5;
                if ($currentlraParent !== null) {
                    var width = $currentlraParent.find('.non-leaf').width() + PADDING_FOR_SYMBOL;

                    $currentlraParent.find('.custom-lra-symbol').css({ width: width });
                    $currentlraParent.find('.custom-lra-line').css({ width: width - 7 });
                }
                /*Custom  Long Right Arrow code end*/

                /*Custom width addition for Left Right Arrow*/
                $currentlraParent = $(this).find('.cursor').parent().parent().hasClass('custom-parent') ? $(this).find('.cursor').parent().parent() : null;
                if ($currentlraParent !== null) {
                    var width = $currentlraParent.find('.non-leaf').width() + PADDING_FOR_SYMBOL;
                    if (width >= 14) {
                        $currentlraParent.find('.custom-symbol').css({ width: width });
                        $currentlraParent.find('.custom-line').css({ width: width - 14 });
                    }
                }
                /*Custom  Left Right Arrow code end*/
                // END - changes done by Zeus
                ...
                // START - changes done by Zeus
                /*Custom width addition for frown*/
                var mathquillParent = $(this).find('.cursor').parents('.mathquill-rendered-math');
                updateFrownSymbolWidth(mathquillParent);
                /*Custom frown code end*/

                /*Custom height addition for floor and ceil symbol*/
                updateCeilFloorSymbolHeight(mathquillParent, 'floor');
                updateCeilFloorSymbolHeight(mathquillParent, 'ceil');
                /*Custom floor and ceil symbol code end*/
    ...
    var Parser = P(function (_, _super, Parser) {
        ...
        var regex = this.regex = function (re) {
            ...
            return Parser(function (stream, onSuccess, onFailure) {
                ...
                if (match) {
                    var result = match[0],
                        sliceLength = result.length;
                    // START - changes done by Zeus
                    // Remove extra '@' character added to latex for getting spacing in regex /^[a-z]+/i applied after detecting '\' on write
                    if (result.indexOf('spacing') >= 0) {
                        sliceLength++;
                    }
                    // END - changes done by Zeus
                    return onSuccess(stream.slice(sliceLength), result);
                }
                else {
                    return onFailure(stream, expected);
                }
    ...
    var Fragment = P(function (_) {
        ...
        function prayWellFormed(parent, leftward, rightward) {
            ...
            pray('leftward is properly set up', (function () {
                ...
                if (!leftward) {
                    if (rightward === parseInt(rightward)) {
                        return parent.endChild[L] === rightward;
                    }
                    else {
                        if (parent.endChild[L] !== rightward) {
                            if (parent.endChild[L] === parseInt(parent.endChild[L])) {
                                return parent.endChild[L] === rightward[L]
                            }
                            else {
                                return parent.endChild[L] === rightward.endChild[L];
                            }
                        }
                        else {
                            return parent.endChild[L] === rightward;
            ...
            pray('rightward is properly set up', (function () {
                ...
                if (!rightward) {
                    if (leftward === parseInt(leftward)) {
                        return parent.endChild[R] === leftward;
                    }
                    else {
                        if (parent.endChild[R] !== leftward) {
                            if (parent.endChild[R] === parseInt(parent.endChild[R])) {
                                return parent.endChild[R] === leftward[R];
                            }
                            else {
                                return parent.endChild[R] === leftward.endChild[R];
                            }
                        }
                        else {
                            return parent.endChild[R] === leftward;
      ...
      blocks = cmd.blocks = Array(numBlocks);
        ...
        */
        ...
        _.html = function () {
            ...
            var cmdClass = this.getCmdClass(cmd.ctrlSeq); // changes done by Zeus
            ...
            for (var i = 0, token = tokens[0]; token; i += 1, token = tokens[i]) {
                ...
                if (token.slice(-2) === '/>') {
                    tokens[i] = token.slice(0, -2) + cmdId + cmdClass + '/>';
                }
                    // top-level open tags
                else if (token.charAt(0) === '<') {
                    pray('not an unmatched top-level close tag', token.charAt(1) !== '/');

                    tokens[i] = token.slice(0, -1) + cmdId + cmdClass + '>';
        ...
        // START - changes done by Zeus
        _.getCmdClass = function(ctrlSeq) {
            var cmdClass = "",
                punctuationMarks = ["\"", "'", ":", ";", "?", "!"];

            if (punctuationMarks.indexOf(ctrlSeq) > -1) {
                cmdClass = " class=punctuation-mark";
            }
            return cmdClass;
        };
        // END - changes done by Zeus
    ...
    ********************************************/
    ...
    textarea = textareaSpan.children();
        ...
        */
        ...
        function setTextareaSelection() {
            ...
            if (cursor.selection) {
                if (!isNaN(Number(cursor.selection.latex()))) {
                    latex = cursor.selection.latex();
                }
                else {
                    latex = '$' + cursor.selection.latex() + '$';
                }
            ...
            // START - changes done by Zeus
            // Latex for sum and prod changes
            if (latex.indexOf('\\sum') > -1 || latex.indexOf('\\prod') > -1)
            {
                latex = latex.replace(/m {/g, 'm ^{');  // Replace "m {" with "m ^{"
                latex = latex.replace(/d {/g, 'd ^{');  // Replace "d {" with "d ^{"
                latex = latex.replace(/(m\^\{[^\}]*\})(\{)/g, '$1_$2');     // Add "_" between "}" and "{" of upper and lower limit
                latex = latex.replace(/(d\^\{[^\}]*\})(\{)/g, '$1_$2');     // Add "_" between "}" and "{" of upper and lower limit
            }
            // END - changes done by Zeus
      ...
      .prepend('<span class="selectable">$' + root.latex() + '$</span>');
        ...
        var textareaManager = manageTextarea(textarea, {
            ...
            paste: function (text) {
                ...
                /* 'text' prepend removed for paste functions. */
                ...
                // START - changes done by Zeus
                text = handleSpace(text);
                if (text && text.indexOf('\\mixedfrac') >= 0) {
                    text = text.replace(/\\spacing@/g, ' ');
                }
                // END - changes done by Zeus
        ...
        textarea.focus(function (e) {
            ...
            if (cursor.selection)
			{
				...
				$(this).parents('.mathquill-editable').mathquill('clearSelection');
			}
            e.stopPropagation();
        });
    ...
    var RootMathBlock = P(MathBlock, function (_, _super) {
        _.latex = function () {
			var returnLatex = _super.latex.call(this).replace(/(\\[a-z]+) (?![a-z])/ig, function($1){
				return $1;
			});
        ...
        // START - changes done by Zeus
        var absRegExWithBracket = /\\text\{abs\}\\left\(/g,
            absRegExWithOutBracket = /\\text\{abs\}/g,
            permutationCombinationRegEx = /(npr|ncr)/g,
            requiredPermutationCombinationRegEx = /\\text\{(npr|ncr)\}/g,
            absSymbolLeftPipeRegEx = /(\\left\|)/g,
            absSymbolRightPipeRegEx = /\\right\|/g,
            leftBracketRegEx = /\\left\(/g,
            rightBracketRegEx = /\\right\)/g,
            numRegEx = /\d/,
            absStartIndex, absEndIndex,
            rightBracketStartIndex, rightBracketEndIndex,
            rightPipeStartIndex, isRightBracket, isLeftBracket,
            leftBracketLastIndex, rightBracketLastIndex;
            while (absRegExWithBracket.test(latex)) {
                absStartIndex = latex.search(absRegExWithBracket);
                absEndIndex = absRegExWithBracket.lastIndex;
                latex = latex.slice(0, absStartIndex) + '\\left\\lpipe' + latex.slice(absEndIndex);

                isLeftBracket = leftBracketRegEx.test(latex);
                ...
                isRightBracket = rightBracketRegEx.test(latex);
                ...
                // 7 is number of latex characters for right bracket..
                rightBracketStartIndex = rightBracketRegEx.lastIndex - 7;
                rightBracketEndIndex = rightBracketRegEx.lastIndex;
                if (rightBracketEndIndex === latex.length) {
                    latex = latex.slice(0, rightBracketStartIndex) + '\\right\\rpipe';
                } else {
                    latex = latex.slice(0, rightBracketStartIndex) + '\\right\\rpipe' + latex.slice(rightBracketEndIndex);
                }
                absRegExWithBracket.lastIndex = leftBracketRegEx.lastIndex = rightBracketRegEx.lastIndex = 0;
            }
            while (absRegExWithOutBracket.test(latex)) {
                absStartIndex = latex.search(absRegExWithOutBracket);
                rightPipeStartIndex = absEndIndex = absRegExWithOutBracket.lastIndex;
                while (numRegEx.test(parseInt(latex[rightPipeStartIndex], 10)) && rightPipeStartIndex !== latex.length) {
                    rightPipeStartIndex++;
                }
                if (rightPipeStartIndex === latex.length) {
                    latex = latex.substring(0, absStartIndex) + '\\left\\lpipe' + latex.substring(absEndIndex, rightPipeStartIndex) + '\\right\\rpipe';
                } else {
                    latex = latex.substring(0, absStartIndex) + '\\left\\lpipe' + latex.substring(absEndIndex, rightPipeStartIndex) + '\\right\\rpipe' + latex.substring(rightPipeStartIndex);
                }
            }
            latex = latex.replace(requiredPermutationCombinationRegEx, '$1')
                .replace(permutationCombinationRegEx, '\\text\{$1\}')
                .replace(absSymbolLeftPipeRegEx, '\\left\\lpipe')
                .replace(absSymbolRightPipeRegEx, '\\right\\rpipe');
            // END - changes done by Zeus
        ...
        _.onKey = function (key, e) {
            ...
            switch (key) {
                ...
                case 'Tab':
                // START - changes done by Zeus
                /* Commented Spacebar to allow user type/paste spaces */
                //case 'Spacebar':

                    /*Commented following lines to avoid cursor movement on Spacebar click*/
                ...
                case 'Enter':
						this.jQ.mathquill('clearSelection');
						this.jQ.mathquill('blur');
						break;
                ...
                case 'PlusMinus':
                    e.preventDefault();
                    if(this.cursor.jQ.length === 0) {
                        this.cursor.show();
                    }
                    if(this.cursor.selection) {
                        this.cursor.deleteSelection();
                    }
                    $target = this.cursor.root.jQ;
                    while(true) {
                        if(!this.cursor[L] && this.cursor.jQ.parent().hasClass('mathquill-editable') &&
                            !this.cursor[R]) {
                            /*If the textarea is empty*/
                            /*Add Unary Operator*/
                            $target.mathquill('latex');
                            $target.mathquill('write','-');
                            this.cursor.show();
                            break;
                        } else if(!this.cursor[L] && this.cursor.jQ.parent().hasClass('mathquill-editable') &&
                            this.cursor[R] && !this.cursor[R].jQ.hasClass('unary-operator')) {
                            /*if cursor is at the start of the textarea and
                            there is no unary operator to the right side of the cursor*/
                            /*Add Unary Operator*/
                            $target.mathquill('latex');
                            $target.mathquill('write','-');
                            this.cursor.show();
                            break;
                        } else if(!this.cursor[L] && this.cursor[R] &&
                            this.cursor[R].jQ.hasClass('unary-operator')) {
                            /*If there is a unary operator to the right side of cursor*/
                            /*Remove Unary-operator*/
                            this.cursor.deleteForward();
                            break;
                        } else if(this.cursor[L] && this.cursor[L].jQ.hasClass('binary-operator') &&
                            !this.cursor[R]) {
                            /*If there is a binary operator to the left side of the cursor and
                             nothing on the right side of the cursor*/
                            /*Add Unary-operator*/
                            $target.mathquill('latex');
                            $target.mathquill('write','-');
                            this.cursor.show();
                            ...
                            this.cursor[R] && this.cursor[R].jQ.hasClass('unary-operator')) {
                            /*If there is binary operator on the left side of the cursor and
                            unary operator on the right side of the cursor*/
                            /*Remove Unary-operator*/
                            this.cursor.deleteForward();
                            break;
                        } else if(this.cursor[L] && this.cursor[L].jQ.hasClass('binary-operator') &&
                            this.cursor[R] && !this.cursor[R].jQ.hasClass('unary-operator')) {
                            /*If there is binary operator on the left side of the cursor and
                            not a unary operator on the right side of the cursor*/
                            /*Add Unary-operator*/
                            $target.mathquill('latex');
                            $target.mathquill('write','-');
                            this.cursor.show();
                            break;
                        } else if(this.cursor[L] && this.cursor[L].jQ.hasClass('unary-operator')) {
                            /*if there is unary operator on the left side of the cursor*/
                            /*Remove Unary-operator*/
                            this.cursor.backspace();
                            break;
                        } else if(this.cursor[L] && this.cursor[L].jQ.hasClass('non-leaf') &&
                            this.cursor[R] && this.cursor[R].jQ.hasClass('non-leaf') &&
                            this.cursor[L].jQ[0].tagName.toLowerCase() === 'span' &&
                            this.cursor[R].jQ[0].tagName.toLowerCase() === 'span') {
                            /*If there are expressions on the left and right side of the cursor*/
                            /*Add Multiplication sign and Unary-operator*/
                            $target.mathquill('latex');
                            $target.mathquill('write','\\cdot');
                            $target.mathquill('latex');
                            $target.mathquill('write','-');
                            this.cursor.show();
                            break;
                        } else if(!this.cursor[L] && this.cursor.jQ.parent() &&
                            this.cursor.jQ.parent().hasClass('hasCursor')) {
                            /*If there is nothing on the left side of the cursor & cursor is
                            inside the non-leaf node, that is, its parent node is non-leaf node*/
                            /*Add unary operator*/
                            $target.mathquill('latex');
                            $target.mathquill('write','-');
                            this.cursor.show();
                            ...
                            leftElement = this.cursor[L].jQ;
                            while(this.cursor[R].jQ !== leftElement) {
...
LatexCmds.fraction = P(MathCommand, function (_, _super) {
    ...
    _.finalizeTree = function () {
        // START - changes done by Zeus
        // up from denominator
        this.endChild[R].up = this.endChild[L];
        // down from numerator
        this.endChild[L].down = this.endChild[R];

        // up from numerator to prevent cursor stuck
        this.endChild[L].up = function(){
            // sum/prod in denominator and frac in its lower limit
            if(this.parent.parent.parent.jQ && this.parent.parent.parent.jQ.hasClass('from')){
                // cursor to upper limit value
                return this.parent.parent.endChild[L];
            }
        }

        // down from denominator to prevent cursor stuck
        this.endChild[R].down = function(){
            // sum/prod in denominator and frac in its upper limit
            if(this.parent.parent.parent.jQ && this.parent.parent.parent.jQ.hasClass('to')){
                // cursor to lower limit value
                return this.parent.parent.endChild[R];
            }
        }
        // END - changes done by Zeus
    };
...
CharCmds['/'] = P(Fraction, function (_, _super) {
    _.createLeftOf = function (cursor) {
        ...
        !(
          ...
          leftward instanceof Summation ||
          leftward instanceof Product ||
...
LatexCmds['√'] = P(MathCommand, function (_, _super) {
    ...
    _.redraw = function () {
        ...
        if ((block.innerHeight() / +block.css('fontSize').slice(0, -2) - .1) < 0) {
            y = 0.9;
        }
        else {
            y = block.innerHeight() / +block.css('fontSize').slice(0, -2) - .1;
        }
...
LatexCmds.nthroot = P(SquareRoot, function (_, _super) {
    ...
    var Bracket = P(MathCommand, function (_, _super) {
      ...
      [open, close]);
        ...
        _.latex = function () {
            /* Latex for 'ceil' and 'floor' functions changed. */
            //if (this.ctrlSeq === '\\left\\⌈ ' && this.end === '\\right\\⌉ ') {
            //    this.ctrlSeq = '\\{ceil}\\left( ';
            //    this.end = '\\right) ';
            //    return this.ctrlSeq + this.endChild[L].latex() + this.end;
            //}
            //else if (this.ctrlSeq === '\\left\\⌊ ' && this.end === '\\right\\⌋ ') {
            //    this.ctrlSeq = '\\{floor}\\left( ';
            //    this.end = '\\right) ';
            //    return this.ctrlSeq + this.endChild[L].latex() + this.end;
            //}
            //else {
    ...
    // START - changes done by Zeus
    /*Custom function for displaying ceil symbol because it was not
    rendered properly in Mac Safari*/
    var CeilSymbol = P(MathCommand, function (_, _super) {
        _.init = function (open, close, ctrlSeq, end) {
            // Changes done by Zeus
            // Replace "object" tag with "svg"
            var htmlTemplate =
                '<span class="non-leaf">' +
                '<span class="custom-ceil-parent">' +
                '<span class="custom-ceil-holder">' +
                '<svg class="custom-ceil" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="none" width="4" height="17" viewBox="0 0 4 17">' +
                '<defs><style>.cls-1 { fill: #333; fill-rule: evenodd; }</style></defs>' +
                '<path d="M4.000,-0.000 L4.000,1.000 L1.000,1.000 L1.000,17.000 L-0.000,17.000 L-0.000,-0.000 L4.000,-0.000 Z" class="cls-1"/>' +
                '</svg>' +
                '</span>' +
                '<span class="non-leaf">&0</span>' +
                '<span class="custom-ceil-holder">' +
                '<svg class="custom-ceil" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="none" width="4" height="17" viewBox="0 0 4 17">' +
                '<defs><style>.cls-1 { fill: #333; fill-rule: evenodd; }</style></defs>' +
                '<path d="M-0.000,-0.000 L-0.000,1.000 L3.000,1.000 L3.000,17.000 L4.000,17.000 L4.000,-0.000 L-0.000,-0.000 Z" class="cls-1"/>' +
                '</svg>' +
                '</span>' +
                '</span>' +
                '</span>';
            _super.init.call(this, '\\left' + ctrlSeq, htmlTemplate, [open, close]);
            this.end = '\\right' + end
        };
        _.jQadd = function () {
            _super.jQadd.apply(this, arguments);
    ...
    /*Custom function for displaying floor symbol because it was not
    rendered properly in Mac Safari*/
    var FloorSymbol = P(MathCommand, function (_, _super) {
        _.init = function (open, close, ctrlSeq, end) {
            // Changes done by Zeus
            // Replace "object" tag with "svg"
            var htmlTemplate =
                '<span class="non-leaf">' +
                '<span class="custom-floor-parent">' +
                '<span class="custom-floor-holder">' +
                '<svg class="custom-floor" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="none" width="4" height="17" viewBox="0 0 4 17">' +
                '<defs><style>.cls-1 { fill: #333; fill-rule: evenodd; }</style></defs>' +
                '<path d="M4.000,17.000 L4.000,16.000 L1.000,16.000 L1.000,-0.000 L-0.000,-0.000 L-0.000,17.000 L4.000,17.000 Z" class="cls-1"/>' +
                '</svg>' +
                '</span>' +
                '<span class="non-leaf">&0</span>' +
                '<span class="custom-floor-holder">' +
                '<svg class="custom-floor" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="none" width="4" height="17" viewBox="0 0 4 17">' +
                '<defs><style>.cls-1 { fill: #333; fill-rule: evenodd; }</style></defs>' +
                '<path d="M-0.000,17.000 L-0.000,16.000 L3.000,16.000 L3.000,-0.000 L4.000,-0.000 L4.000,17.000 L-0.000,17.000 Z" class="cls-1"/>' +
                '</svg>' +
                '</span>' +
                '</span>' +
                '</span>';
    ...
    LatexCmds.left = P(MathCommand, function (_) {
        _.parser = function () {
            ...
            return optWhitespace.then(regex(/^(?:[([|]|\\\{|\\lceil|\\lfloor|\\lpipe)/))
      .then(function (open) {
...
LatexCmds.lang = bind(Bracket, '&lang;', '&rang;', '\\langle ', '\\rangle ');
    ...
    /* Added for 'ceil' and 'floor' functions. */
    //LatexCmds.lceil = CharCmds['⌈'] = bind(Bracket, '⌈', '⌉', '\\⌈ ', '\\⌉ ');
    //LatexCmds.lfloor = CharCmds['⌊'] = bind(Bracket, '⌊', '⌋', '\\⌊ ', '\\⌋ ');
    // START - changes done by Zeus
    CharCmds['lceil'] = LatexCmds.lceil = bind(CeilSymbol, '&#x2308;', '&#x2309;', '\\lceil', '\\rceil');
    CharCmds['lfloor'] = LatexCmds.lfloor = bind(FloorSymbol, '&#x230A;', '&#x230B;', '\\lfloor', '\\rfloor');
    LatexCmds.abs = CharCmds['|'] = CharCmds['lpipe'] = CharCmds['rpipe'] = CharCmds['abs'] = LatexCmds.abs = bind(Bracket, '&#x1c0;', '&#x1c0;', '\\lpipe', '\\rpipe');
    // END - changes done by Zeus
    ...
    // START - changes done by Zeus
    CharCmds['rceil'] = LatexCmds.rceil = bind(CeilSymbol, '&#x2308;', '&#x2309;', '\\lceil', '\\rceil');
    CharCmds['rfloor'] = LatexCmds.rfloor = bind(FloorSymbol, '&#x230A;', '&#x230B;', '\\lfloor', '\\rfloor');
    // END - changes done by Zeus
...
// START - changes done by Zeus
/*pipe symbol is now used for absolute sign*/
//     var Pipes =
// LatexCmds.lpipe =
// LatexCmds.rpipe =
...
LatexCmds.vector = P(MathCommand, function (_, _super) {
    ...
    *********************************/
    ...
    LatexCmds.f = bind(Symbol, 'f', '<var>f</var>');
    ...
    // START - changes done by Zeus
    /*Custom function for using frown symbol as arc*/
    var FrownSymbol = P(MathCommand, function (_, _super) {
        _.init = function (ch, html) {
            // Changes done by Zeus
            // Replace "object" tag with "svg"
            var htmlTemplate = '<span class="custom-frown-parent"><span class="custom-frown-holder"><svg class="custom-frown" \
            xmlns="http://www.w3.org/2000/svg" xml:space="preserve" preserveAspectRatio="none" height="9px" width="9px" style="shape-rendering:geometricPrecision; \
            text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd" viewBox="0 0 110 30.0004" \
            xmlns:xlink="http://www.w3.org/1999/xlink"><g id="Layer_x0020_1"><metadata id="CorelCorpID_0Corel-Layer"/><path class="fil0" d="M0.191341 \
            28.0114c13.7376,-16.6798 32.8221,-26.952 57.1945,-27.0169 19.2404,-0.0507881 40.7344,7.79539 52.7606,27.0063 0,0 -18.3133,-17.4487 -52.4677,\
              -18.1833 -31.8123,-0.683868 -57.4874,18.194 -57.4874,18.194z"/></g></svg></span><span class="non-leaf">&0</span></span>';
            _super.init.call(this, ch, htmlTemplate);
        };
    });
    // END - changes done by Zeus

    var LongRightArrowSymbol = P(MathCommand, function (_, _super) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="custom-lra-parent"><span class="custom-lra-symbol"><span class="custom-lra-line"></span><span class="custom-lra-head"></span></span>';
            htmlTemplate += '<span class="non-leaf">&0</span></span>'
            _super.init.call(this, ch, htmlTemplate);
        };
    });

    var LeftRightArrowSymbol = P(MathCommand, function (_, _super) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="custom-parent "><span class="custom-symbol"><span class="custom-left-head"></span><span class="custom-line"></span><span class="custom-right-head"></span></span>';
            htmlTemplate += '<span class="non-leaf">&0</span></span>'
            _super.init.call(this, ch, htmlTemplate);
        };
    });

    var VectorSymbol = P(MathCommand, function (_, _super) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="custom-parent "><span class="custom-symbol"><span class="custom-line"></span><span class="custom-vector-right-head"></span></span>';
            htmlTemplate += '<span class="non-leaf">&0</span></span>'
            _super.init.call(this, ch, htmlTemplate);
        };
    });

    var Summation = P(MathCommand, function (_, _super) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="mathquill-main-block"><span class="large-operator non-leaf"><span class="to"><span>&0</span></span><big class="bigSymbol">∑</big><span class="from"><span>&1</span></span></span></span>';
            _super.init.call(this, ch, htmlTemplate);
        };
        _.finalizeTree = function () {
            this.endChild[R].up = this.endChild[L];
    ...
    var Product = P(MathCommand, function (_, _super) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="mathquill-main-block"><span class="large-operator non-leaf"><span class="to"><span>&0</span></span><big class="bigSymbol">∏</big><span class="from"><span>&1</span></span></span></span>';
            _super.init.call(this, ch, htmlTemplate);
        };
    ...
    // START - changes done by Zeus
    LatexCmds['spacing'] = CharCmds[' '] = bind(VanillaSymbol, "\\spacing@", '&#x20;'); // Add latex "\\spacing@" for space
    // END - changes done by Zeus
...
LatexCmds.forall = P(VanillaSymbol, function (_, _super) {
    ...
    // START - changes done by Zeus
    var punctuationMark = P(Symbol, function (_, _super) {
        _.init = function (ctrlSeq, html, text) {
            _super.init.call(this,
                ctrlSeq, '<span class="punctuation-mark">' + html + '</span>', text
            );
        };
    });
    // END - changes done by Zeus
    ...
    var PlusMinus = P(BinaryOperator, function (_) {
        ...
        _.respace = function () {
            // START - changes done by Zeus
            if (!this[L] && !this[R] && this.ctrlSeq === '-') {
                this.jQ[0].className = 'unary-operator';
            } else if (this[L] && this[L] instanceof BinaryOperator &&
                !this[R] && this.ctrlSeq === '-') {
                this.jQ[0].className = 'unary-operator';
            } else if(!this[L] && this[R] && !this[R].jQ.hasClass('unary-operator') &&
                this.ctrlSeq === '-') {
                this.jQ[0].className = 'unary-operator';
            }
            else if (this[L] instanceof BinaryOperator &&
  ...
  bind(BinaryOperator, '\\cdot ', '&middot;');
    ...
    // START - changes done by Zeus
    LatexCmds['\"'] = bind(punctuationMark, '\"', '\"');
    LatexCmds.prime = bind(punctuationMark, "'", '&prime;');
    LatexCmds[':'] = bind(punctuationMark, ':', ':');
    LatexCmds[';'] = bind(punctuationMark, ';', ';');
    LatexCmds['?'] = bind(punctuationMark, '?', '?');
    LatexCmds['!'] = bind(punctuationMark, '!', '!');
    CharCmds["'"] = bind(punctuationMark, "'", '&prime;');
    // END - changes done by Zeus
...
// START - changes done by Zeus
    LatexCmds['∑'] = LatexCmds.sum = LatexCmds.summation = bind(Summation, '\\sum ', '&sum;');
    LatexCmds['∏'] = LatexCmds.prod = LatexCmds.product = bind(Product, '\\prod ', '&prod;');
...
// END - changes done by Zeus
    ...
    // START - Code changed by Zeus
    LatexCmds.mix = LatexCmds.mixedfrac = P(MathCommand, function (_, _super) {
        _.ctrlSeq = '\\mixedfrac';
        _.htmlTemplate =
            '<span class="mixed-fraction-holder">'
            + '<span class="whole-fraction"><span>&0</span></span>'
            + '<span class="fraction non-leaf">'
        ...
        _.textTemplate = ['(', '&mix;', ')'];
        _.finalizeTree = function () {
            // Normally first child is taken for leftmost, i.e. this.endChild[L] and in mixedFraction it's whole part of it
            // So for up direction navigation numerator is taken which is right side of whole number
            var numerator = this.endChild[L][R];
            this.endChild[R].up = numerator;
            numerator.down = this.endChild[R];

            // up from numerator to prevent cursor stuck
            numerator.up = function(){
                // sum/prod in denominator and mixed fraction in its lower limit
                if(this.parent.parent.jQ.parent() && this.parent.parent.jQ.parent().hasClass('from')){
                    // cursor to upper limit value
                    return this.parent.parent.parent.endChild[L];
                }
            }

            // down from denominator to prevent cursor stuck
            this.endChild[R].down = function(){
                // sum/prod in denominator and mixed fraction in its upper limit
                if(this.parent.parent.jQ.parent() && this.parent.parent.jQ.parent().hasClass('to')){
                    // cursor to lower limit value
                    return this.parent.parent.endChild[R];
                }
            }
...
LatexCmds.complexplane = LatexCmds.Complexplane = LatexCmds.ComplexPlane =
  ...
  bind(VanillaSymbol, '\\mathbb{H}', '&#8461;');
    ...
    */
    ...
    LatexCmds.frown = bind(FrownSymbol, '\\frown ', '&#8994;');
    ...
    LatexCmds.longrightarrow = bind(LongRightArrowSymbol, '\\longrightarrow ', '&#8594;');
    LatexCmds.Longleftarrow = bind(VanillaSymbol, '\\Longleftarrow ', '&#8656;');
    LatexCmds.Longrightarrow = bind(VanillaSymbol, '\\Longrightarrow ', '&#8658;');
    ...
    LatexCmds.nwvector = bind(VectorSymbol, '\\nwvector ', '&#8598;');
    ...
    /* VanillaSymbol for 'ceil' and 'floor' functions removed. */
...
LatexCmds.darr = LatexCmds.dnarr = LatexCmds.dnarrow = LatexCmds.downarrow =
  ...
  bind(LeftRightArrowSymbol, '\\leftrightarrow ', '&harr;');
...
LatexCmds.lim = NonItalicizedFunction;
    ...
    var Cursor = P(Point, function (_) {
        ...
        _.moveDirWithin = function (dir, block) {
            ...
            if (this[dir]) {
                ...
                if (this.parent[dir])
                        this.insAtDirEnd(-dir, this.parent[dir]);
        ...
        */
        ...
        function moveUpDown(self, dir) {
            ...
            var cursorParent = self.jQ.parent(),
                cursorMainBlock = self.jQ.parents('.mathquill-main-block'),
                cursorFromBlock = self.jQ.parents('.from'),
                cursorToBlock = self.jQ.parents('.to');
            ...
            if (self[R][dir]) self.insAtLeftEnd(self[R][dir]);
                ...
                do {
                        ...
                        if (prop === false || prop instanceof MathBlock) {
                            ...
                            if (prop instanceof MathBlock) {
                                ...
                                if (cached) {
                                    ...
                                    /*ie js error fix*/
                                    if (typeof offset(self) === 'undefined') {
                                        self.jQ = self._jQ;
                                    } /*end of - ie js error fix*/
                    ...
                    // START - changes done by Zeus
                    //movement from 'from' to 'right parent'
                    else if (cursorMainBlock.length > 0 && cursorFromBlock.length > 0 && dir === 'up' &&
                        (cursorParent.children().length - 1 === cursorParent.children('.cursor').index()) &&
                        self.jQ.parent().children('.mathquill-main-block').length === 0) {
                        self.moveRight(cursorMainBlock);
                        self.moveRight(cursorMainBlock);
                        break;
                    }
                    //movement from 'right parent' to 'main block'
                    else if (self[L] && self[L].jQ.hasClass('mathquill-main-block')) {
                        if (dir === 'up') {
                            self.insAtLeftEnd(self[L].endChild[L]);
                        }
                        else {
                            self.insAtRightEnd(self[L].endChild[R]);
                        }
                        break;
                    }
                    //movement from 'left parent' to 'main block'
                    else if (self[R] && self[R].jQ.hasClass('mathquill-main-block')) {
                        if (dir === 'up') {
                            self.insAtRightEnd(self[R].endChild[L]);
                        }
                        else {
                            self.insAtLeftEnd(self[R].endChild[R]);
                        }
                        break;
                    }
                    else if (cursorMainBlock.length > 0 && cursorToBlock.length > 0 && dir==='down') {
                        self.moveLeftWithin(cursorMainBlock);
                        break;
                    }
                    // END - changes done by Zeus
        ...
        _.writeLatex = function (latex) {
            ...
            if (block) {
                ...
                var mathquillParent = self.jQ.parents('.mathquill-rendered-math'),
                    customLraParent = mathquillParent.find('.custom-lra-parent'),
                    customLraLine = mathquillParent.find('.custom-lra-line'),
                    customLraSymbol = mathquillParent.find('.custom-lra-symbol'),
                    customLraNonLeafWidth = customLraParent.find('.non-leaf').width(),
                    customLraArrowWidth = 7;

                var customParent = mathquillParent.find('.custom-parent'),
                    customLine = mathquillParent.find('.custom-line'),
                    customSymbol = mathquillParent.find('.custom-symbol'),
                    customNonLeafWidth = customParent.find('.non-leaf').width(),
                    customArrowWidth = 14;

                if (customLraLine.length > 0) {
                    customLraSymbol.width(customLraNonLeafWidth);
                    customLraLine.width(customLraNonLeafWidth - customLraArrowWidth);
                }

                if (customLine.length > 0 && customNonLeafWidth >= customArrowWidth) {
                    customSymbol.width(customNonLeafWidth);
                    customLine.width(customNonLeafWidth - customArrowWidth);
                }
                ...
                // START - Code changed by Zeus
                if(self[L]) {
                    self[L].finalizeInsert();
                }
                // END - Code changed by Zeus
                block.finalizeInsert();

                // START - changes done by Zeus
                // Instead of calculating width of frown for the first frown element,
                // Calculate and apply width for each frown element
                updateFrownSymbolWidth(mathquillParent);
    ...
    ********************************************************/
    // START - changes done by Zeus
    /* Function to be called in jQuery.fn.mathquill and on paste to handle spaces*/
    var handleSpace = function (latex) {
        latex = latex.toString();
        // Replace spaces by special character which is not used for any other purpose
        latex = latex.replace(/ /gi, '\\spacing@');
        // Exclude cases for non italicized functions where extra space is added by mathquill / in keyboard.js(json)
        var i,
            nonItalicizedFunc = ['ln', 'log', 'min', 'max', 'mod', 'lcm', 'gcd', 'theta', 'phi', 'pi',
                                 'le', 'ge', 'bigtriangleup', 'pm', 'mp', 'approx', 'angle',
                                 'sim', 'ne', 'parallel', 'perp', 'circ', 'arrow',
                                 'frown', 'prime', 'cong', 'line', 'rightharpoonup', 'cdot', "nwvector"],
            trigFunc = ['sin', 'cos', 'tan', 'sec', 'cosec', 'csc', 'cotan', 'cot'];
        for (i in trigFunc) {
            latex = latex.replace(new RegExp(trigFunc[i] + '\\\\spacing@', 'g'), trigFunc[i] + ' ')
                         .replace(new RegExp(trigFunc[i] + 'h' + '\\\\spacing@', 'g'), trigFunc[i] + 'h' + ' ')
                         .replace(new RegExp('a' + trigFunc[i] + '\\\\spacing@', 'g'), 'a' + trigFunc[i] + ' ')
                         .replace(new RegExp('arc' + trigFunc[i] + '\\\\spacing@', 'g'), 'arc' + trigFunc[i] + ' ')
                         .replace(new RegExp('a' + trigFunc[i] + 'h' + '\\\\spacing@', 'g'), 'a' + trigFunc[i] + 'h' + ' ')
                         .replace(new RegExp('arc' + trigFunc[i] + 'h' + '\\\\spacing@', 'g'), 'arc' + trigFunc[i] + 'h' + ' ');
        }
        for (i in nonItalicizedFunc) {
            latex = latex.replace(new RegExp(nonItalicizedFunc[i] + '\\\\spacing@', 'g'), nonItalicizedFunc[i] + ' ');
        }

        if (latex && latex.indexOf('\\sum') >= 0) {
            latex = latex.replace(/(m|m\\spacing@)\^{/g, 'm {');    // Replace "m\spacing@^{" OR "m^{" with "m {"
            latex = latex.replace(/}_{/g, '}{');                    // Replace "}_{" with "}{"
        }
        if (latex && latex.indexOf('\\prod') >= 0) {
            latex = latex.replace(/(d|d\\spacing@)\^{/g, 'd {');    // Replace "d\spacing@^{" OR "d^{" with "d {"
            latex = latex.replace(/}_{/g, '}{');                    // Replace "}_{" with "}{"
        }

        // Fix for text in calculator Not giving math error eg. "5 +"
        if (latex.indexOf('\\text') >= 0){
            var re = /\\text\{([^}]+)\}/g,                          // match string "\text{anything without'}'}"
                matches = latex.match(re),
                editedText;

            this._.each(matches, function(match) {
                editedText = match.replace(/\\spacing@/g, ' ');     // Replace \spacing@ to ' ' within match
                latex = latex.replace(match, editedText);
            }, this);
        }
      ...
      if (block && block.textarea)
        ...
        block = blockId && MathElement[blockId];
            ...
            case 'latex':
                ...
                var blockId = $(this).attr(mqBlockId),
      block = blockId && MathElement[blockId];
                var latexData = block && block.latex();
                // START - changes done by Zeus
                // Latex for sum and prod changes
                if (latexData && latexData.indexOf('\\sum') >= 0) {
                    latexData = latexData.replace(/m {/g, 'm^{');           // Replace "m {" with "m^{"
                    latexData = latexData.replace(/(m\^\{[^\}]*\})(\{)/g, '$1_$2');     // Add "_" between "}" and "{" of upper and lower limit
                    if (latexData && latexData.indexOf('\\prod') >= 0) {
                        latexData = latexData.replace(/d {/g, 'd^{');       // Replace "m {" with "m^{"
                        latexData = latexData.replace(/(d\^\{[^\}]*\})(\{)/g, '$1_$2'); // Add "_" between "}" and "{" of upper and lower limit
                        return latexData;
                    }
                    return latexData;
                }
                if (latexData && latexData.indexOf('\\prod') >= 0) {
                    latexData = latexData.replace(/d {/g, 'd^{');           // Replace "d {" with "d^{"
                    latexData = latexData.replace(/(d\^\{[^\}]*\})(\{)/g, '$1_$2');     // Add "_" between "}" and "{" of upper and lower limit
                    if (latexData && latexData.indexOf('\\sum') >= 0) {
                        latexData = latexData.replace(/m {/g, 'm^{');       // Replace "m {" with "m^{"
                        latexData = latexData.replace(/(m\^\{[^\}]*\})(\{)/g, '$1_$2'); // Add "_" between "}" and "{" of upper and lower limit
